...
<div>
	...
	<div v-else-if="c.type === 'buttons'" :class="['_buttons', c.className ]" :style="{ justifyContent: align }">
		<MkButton v-for="button in c.buttons" :primary="button.primary" :style="{ cursor: pointer }" :className="button.className" :rounded="button.rounded" :disabled="button.disabled" inline :small="size === 'small'" @click="button.onClick" @touchstart="c.onTouchDown" @touchend="c.onTouchUp">{{ button.text }}</MkButton>
	</div>
