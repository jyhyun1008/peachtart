...
<div>
	...
	<div v-else-if="c.type === 'text' && c.img !== 'none'" :class="[{ [$style.fontSerif]: c.font === 'serif', [$style.fontMonospace]: c.font === 'monospace' }, c.className]" :style="{ height: c.height+'px', backgroundImage: 'url('+c.img+')', backgroundSize: 'cover', fontSize: c.size ? `${c.size * 100}%` : null, fontWeight: c.bold ? 'bold' : null, color: c.color ?? null }">{{ c.text }}</div>
