...
<div>
	...
	<MkButton v-else-if="c.type === 'button'" :primary="c.primary" :className="c.className" :style="{ cursor: pointer }" :rounded="c.rounded" :disabled="c.disabled" :small="size === 'small'" inline @click="c.onClick" @touchstart="c.onTouchDown" @touchend="c.onTouchUp">{{ c.text }}</MkButton>
	<div v-else-if="c.type === 'buttons'" :class="['_buttons', c.className ]" :style="{ justifyContent: align }">
		<MkButton v-for="button in c.buttons" :primary="button.primary" :style="{ cursor: pointer }" :className="button.className" :rounded="button.rounded" :disabled="button.disabled" inline :small="size === 'small'" @click="button.onClick" @touchstart="button.onTouchDown" @touchend="button.onTouchUp">{{ button.text }}</MkButton>
	</div>
