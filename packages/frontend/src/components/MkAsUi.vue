...
<div>
	...
	<div v-else-if="c.type === 'text' && c.img !== 'none'" :class="[{ [$style.fontSerif]: c.font === 'serif', [$style.fontMonospace]: c.font === 'monospace' }, c.className]" :style="{ c.css height: c.height, backgroundImage: 'url('+c.img+')', backgroundSize: 'cover', fontSize: c.size ? `${c.size * 100}%` : null, fontWeight: c.bold ? 'bold' : null, color: c.color ?? null }">{{ c.text }}</div>
	<span v-else-if="c.type === 'text'" :class="[{ [$style.fontSerif]: c.font === 'serif', [$style.fontMonospace]: c.font === 'monospace' }, c.className]" :style="{ c.css fontSize: c.size ? `${c.size * 100}%` : null, fontWeight: c.bold ? 'bold' : null, color: c.color ?? null }">{{ c.text }}</span>
	<Mfm v-else-if="c.type === 'mfm'" :class="[{ [$style.fontSerif]: c.font === 'serif', [$style.fontMonospace]: c.font === 'monospace' }, c.className]" :style="{ c.css fontSize: c.size ? `${c.size * 100}%` : null, fontWeight: c.bold ? 'bold' : null, color: c.color ?? null }" :text="c.text"/>
	<MkButton v-else-if="c.type === 'button'" :primary="c.primary" :className="c.className" :style="{ c.css cursor: pointer }" :rounded="c.rounded" :disabled="c.disabled" :small="size === 'small'" inline @click="c.onClick" @touchstart="c.onTouchDown" @touchend="c.onTouchUp" @touchmove="c.onTouchMove">{{ c.text }}</MkButton>
	<div v-else-if="c.type === 'buttons'" :class="['_buttons', c.className ]" :style="{ c.css justifyContent: align }">
		<MkButton v-for="button in c.buttons" :primary="button.primary" :style="{ c.css cursor: pointer }" :className="button.className" :rounded="button.rounded" :disabled="button.disabled" inline :small="size === 'small'" @click="button.onClick" @touchstart="button.onTouchDown" @touchend="button.onTouchUp" @touchmove="button.onTouchMove">{{ button.text }}</MkButton>
	</div>
	...
	<div v-else-if="c.type === 'container'" :class="[$style.container, { [$style.fontSerif]: c.font === 'serif', [$style.fontMonospace]: c.font === 'monospace' }]" :style="{ c.css height: c.height ?? auto, textAlign: c.align ?? null, backgroundColor: c.bgColor ?? null, color: c.fgColor ?? null, borderWidth: c.borderWidth ? `${c.borderWidth}px` : 0, borderColor: c.borderColor ?? 'var(--divider)', padding: c.padding ? `${c.padding}px` : 0, borderRadius: c.rounded ? '8px' : 0 }">
		<div :class="['_container', c.className]" :style="{ c.css backgroundImage: 'url('+c.img+')', height: 'inherit', backgroundSize: 'cover' }">
			<template v-for="child in c.children" :key="child">
