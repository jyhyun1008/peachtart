...
export type AsUiButton = AsUiComponentBase & {
	...
	onTouchDown?: () => Promise<void>;
	onTouchMove?: () => Promise<void>;
	onTouchUp?: () => Promise<void>;
	primary?: boolean;
	rounded?: boolean;
	disabled?: boolean;
	className?: string;
...
function getButtonOptions(def: values.Value | undefined, call: (fn: values.VFn, args: values.Value[]) => Promise<values.Value>): Options<AsUiButton> {
	...
	return {
		...
		onTouchDown: async () => {
			if (onTouchDown) await call(onTouchDown, []);
		},
		onTouchMove: async () => {
			if (onTouchMove) await call(onTouchMove, []);
		},
		onTouchUp: async () => {
			if (onTouchUp) await call(onTouchUp, []);
		},
